...
app.get("/sitemap.xml", async (c) => {
  ...
  const items = [
    { loc: `${origin}/`, lastmod: (/* @__PURE__ */ new Date()).toISOString(), changefreq: "daily", priority: "1.0" },
    ...news.results.map((r) => ({
...
app.get("/news/:slug", async (c) => {
  ...
  const title2 = clamp(row.title, 70) || "Tin tức";
  const raw = (row.meta_description || row.content || "").replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim();
  ...
  const robots = row.published_at ? "index, follow, max-snippet:-1, max-image-preview:large, max-video-preview:-1" : "noindex, nofollow";
  const jsonLd = {
  ...
  return new HTMLRewriter().on("title", { element(el) {
    ...
    element(el) {
      ...
      el.append(`<meta property="og:title" content="${esc(title2)}">`, { html: true });
      ...
      el.append(`<meta name="twitter:title" content="${esc(title2)}">`, { html: true });
...
app.get("/products/:slug", async (c) => {
  ...
  if (!row) return baseRes;
  const title2 = clamp(row.title, 70) || "Sản phẩm";
  const raw = (row.description || row.content || "").replace(/<[^>]*>/g, " ").replace(/\s+/g, " ").trim();
  const description = clamp(raw, 160) || "ALLXONE: Tin tức & sản phẩm.";
  const image = absolute(url, row.image_url || "/og-default.jpg");
