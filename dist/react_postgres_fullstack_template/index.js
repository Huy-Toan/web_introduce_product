...
booksRouter.get("/", async (c) => {
  ...
  try {
    if (c.env.DB_AVAILABLE) {
      ...
      console.log("Database results:", books.length);
      return c.json({
        books,
        source: "database",
        count: books.length,
...
booksRouter.post("/", async (c) => {
  try {
    ...
    const { title: title2, author, genre, description, image_url, published_year } = await c.req.json();
    ...
    const result = await c.env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `).bind(title2, author, genre, description || null, image_url || null, published_year || null).run();
    if (result.success) {
...
booksRouter.put("/:id", async (c) => {
  ...
  try {
    ...
    const { title: title2, author, genre, description, image_url, published_year } = updates;
    const result = await c.env.DB.prepare(`
      UPDATE books 
      SET title = ?, author = ?, genre = ?, description = ?, image_url = ?, published_year = ?
      WHERE id = ?
    `).bind(title2, author, genre, description, image_url, published_year, bookId).run();
    if (result.changes > 0) {
...
booksRouter.delete("/:id", async (c) => {
  ...
  try {
    ...
    const result = await c.env.DB.prepare("DELETE FROM books WHERE id = ?").bind(bookId).run();
    if (result.changes > 0) {
