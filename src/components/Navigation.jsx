...
return (
  <nav className="bg-white shadow-sm border-b border-gray-200 fixed top-0 left-0 right-0 z-50">
    ...
    {/* Mobile Menu - Đưa ra ngoài container chính */}
    {isMobileMenuOpen && (
      <div className="md:hidden bg-white border-t border-gray-200 w-full"> {/* Thêm bg-white và w-full */}
        <div className="px-4 py-2"> {/* Container riêng cho mobile menu */}
          <div className="space-y-1">
            {["home", "about","product", "news", "contact"].map((page) => (
              <button
                key={page}
                onClick={() => handlePageNavigation(page)}
                className="w-full text-left px-4 py-3 text-sm text-gray-600 hover:bg-gray-50 rounded-md"
              >
                {page === "home" ? "Home" : page === "about" ? "About Us" : page ==="contact" ? "Contact" : page === "news" ? "News" : "Products"}
              </button>
            ))}
