...
const FieldFormModal = ({ isOpen, onClose, onSubmit, initialData = {} }) => {
  ...
  // (tuỳ chọn) nếu muốn lưu thêm HTML đã render (BE không bắt buộc)
  ...
  // Ref tới EditorMd để refresh khi modal mở
  ...
  useEffect(() => {
    if (initialData && isOpen) {
      ...
      // đảm bảo layout editor trong modal
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate bắt buộc: name + content
    if (!form.name?.trim()) {
    ...
    try {
      ...
      // (tuỳ chọn) gửi thêm HTML nếu BE muốn:
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-5xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b">
          <h3 className="text-xl font-semibold">
            {initialData.id ? "Chỉnh sửa Lĩnh vực" : "Thêm Lĩnh vực mới"}
          </h3>
          <button
            onClick={onClose}
          ...
          <div>
            ...
            <div className="relative">
              <EditorMd
                ...
                onReady={() => editorRef.current?.refresh?.()}
              />
              {isUploading && (
