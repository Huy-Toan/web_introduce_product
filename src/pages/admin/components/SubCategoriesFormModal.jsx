...
/** i18n labels */
const LABELS = {
  vi: { parent: 'Danh mục cha', parent_ph: '— Chọn danh mục cha —', name: 'Tên danh mục con', name_ph: 'Nhập tên danh mục con', description: 'Mô tả', description_ph: 'Nhập mô tả danh mục con', autoTranslate: 'Tự dịch từ VI', name_any: (lc) => `Tên (${lc})`, name_any_ph: (lc) => `Name (${lc})`, desc_any: (lc) => `Mô tả (${lc})`, desc_any_ph: (lc) => `Description (${lc})`, slug: 'Slug', slug_ph: 'vd: trai-cay-tuoi' },
  en: { parent: 'Parent category', parent_ph: '— Select parent category —', name: 'Subcategory name', name_ph: 'Enter subcategory name', description: 'Description', description_ph: 'Enter subcategory description', autoTranslate: 'Auto-translate from VI', name_any: (lc) => `Name (${lc})`, name_any_ph: (lc) => `Name (${lc})`, desc_any: (lc) => `Description (${lc})`, desc_any_ph: (lc) => `Description (${lc})`, slug: 'Slug', slug_ph: 'e.g. fresh-fruits' },
  ja: { parent: '親カテゴリ', parent_ph: '— 親カテゴリを選択 —', name: 'サブカテゴリ名', name_ph: 'サブカテゴリ名を入力', description: '説明', description_ph: 'サブカテゴリの説明を入力', autoTranslate: 'VI から自動翻訳', name_any: (lc) => `名称 (${lc})`, name_any_ph: (lc) => `名称 (${lc})`, desc_any: (lc) => `説明 (${lc})`, desc_any_ph: (lc) => `説明 (${lc})`, slug: 'スラッグ', slug_ph: '例: fresh-fruits' },
  ko: { parent: '상위 카테고리', parent_ph: '— 상위 카테고리 선택 —', name: '하위 카테고리명', name_ph: '하위 카테고리명을 입력하세요', description: '설명', description_ph: '하위 카테고리 설명을 입력하세요', autoTranslate: '베트남어에서 자동 번역', name_any: (lc) => `이름 (${lc})`, name_any_ph: (lc) => `이름 (${lc})`, desc_any: (lc) => `설명 (${lc})`, desc_any_ph: (lc) => `설명 (${lc})`, slug: '슬러그', slug_ph: '예: fresh-fruits' },
  zh: { parent: '父级分类', parent_ph: '— 选择父级分类 —', name: '子分类名称', name_ph: '输入子分类名称', description: '描述', description_ph: '输入子分类描述', autoTranslate: '从越南语自动翻译', name_any: (lc) => `名称 (${lc})`, name_any_ph: (lc) => `名称 (${lc})`, desc_any: (lc) => `描述 (${lc})`, desc_any_ph: (lc) => `描述 (${lc})`, slug: '短链接', slug_ph: '例如: fresh-fruits' },
  fr: { parent: 'Catégorie parente', parent_ph: '— Sélectionner la catégorie parente —', name: 'Nom de la sous-catégorie', name_ph: 'Saisir le nom de la sous-catégorie', description: 'Description', description_ph: 'Saisir la description de la sous-catégorie', autoTranslate: 'Traduire automatiquement depuis le VI', name_any: (lc) => `Nom (${lc})`, name_any_ph: (lc) => `Nom (${lc})`, desc_any: (lc) => `Description (${lc})`, desc_any_ph: (lc) => `Description (${lc})`, slug: 'Slug', slug_ph: 'ex. fresh-fruits' },
  de: { parent: 'Übergeordnete Kategorie', parent_ph: '— Übergeordnete Kategorie wählen —', name: 'Unterkategoriename', name_ph: 'Unterkategorienamen eingeben', description: 'Beschreibung', description_ph: 'Beschreibung der Unterkategorie eingeben', autoTranslate: 'Automatisch aus VI übersetzen', name_any: (lc) => `Name (${lc})`, name_any_ph: (lc) => `Name (${lc})`, desc_any: (lc) => `Beschreibung (${lc})`, desc_any_ph: (lc) => `Beschreibung (${lc})`, slug: 'Slug', slug_ph: 'z. B. fresh-fruits' },
};
const L = (lc, key, ...args) =>
...
const SubCategoriesFormModal = ({ isOpen, onClose, onSubmit, initialData = {} }) => {
  ...
  // Ảnh: dùng cờ removedImage như AboutForm
  const [removedImage, setRemovedImage] = useState(false);

  // Base (VI)
  const [form, setForm] = useState({ parent_id: '', name: '', slug: '', description: '', image_url: '' });
  ...
  const [imagePreview, setImagePreview] = useState('');    // preview UI

  const [isUploading, setIsUploading] = useState(false);
  const [slugError, setSlugError] = useState('');
  const [slugErrorsTr, setSlugErrorsTr] = useState({});
  ...
  useEffect(() => {
    ...
    setImageFile(null);
    setRemovedImage(false);         // ⭐ reset cờ xoá ảnh
    setSlugError('');
    setSlugErrorsTr({});
      ...
      try {
        ...
        if (j?.translations && typeof j.translations === 'object') {
          ...
          for (const [lc, v] of Object.entries(j.translations)) {
            if (lc === 'vi') continue;
            apiTr[lc] = { name: v?.name || '', slug: v?.slug || '', description: v?.description || '' };
          }
          setTranslations((prev) => ({ ...apiTr, ...prev }));
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      ...
      for (const lc of targets) {
        ...
        try {
          if (srcName) {
            const r = await fetch('/api/translate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: srcName, source: 'vi', target: lc }) });
            if (r.ok) nameTranslated = (await r.json())?.translated || '';
          }
          if (srcDesc) {
            const r2 = await fetch('/api/translate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: srcDesc, source: 'vi', target: lc }) });
  ...
  const canAddLocales = useMemo(() => ALL_LOCALES.filter((lc) => !openLocales.includes(lc)), [openLocales]);
  ...
  const handleTrChange = (lc, key, value) => {
    setTranslations((prev) => ({ ...prev, [lc]: { ...(prev[lc] || { name: '', slug: '', description: '' }), [key]: value } }));
  ...
  const handleImageChange = (e) => {
    ...
    setImageFile(file);
    setRemovedImage(false); // đã chọn ảnh mới -> không phải xoá
    const reader = new FileReader();
    reader.onload = (evt) => setImagePreview(String(evt.target?.result || ''));
    reader.readAsDataURL(file);
  };

  /** Xoá ảnh (tận dụng sẵn) */
  const removeImage = () => {
    setImageFile(null);
    setImagePreview('');
    setForm((prev) => ({ ...prev, image_url: '' }));
    setRemovedImage(true); // đánh dấu xoá
  };
  ...
  const uploadImage = async (file) => {
    ...
    return { image_key: data.image_key, previewUrl: data.displayUrl || data.url };
  };
  ...
  const handleSubmit = async (e) => {
    ...
    // validate bắt buộc
    if (!form.parent_id) return alert(L('vi', 'parent') + ' *');
    if (!form.name?.trim()) return alert(L('vi', 'name') + ' *');
    ...
    for (const lc of Object.keys(slugErrorsTr)) {
      if (slugErrorsTr[lc]) return alert(`Slug (${lc.toUpperCase()}) không hợp lệ.`);
    }
    ...
    try {
      ...
      // ===== Ảnh: 3 nhánh (giống AboutForm) =====
      if (imageFile) {
        const { image_key } = await uploadImage(imageFile);     // ảnh mới
        finalForm.image_url = image_key;
      } else if (isEditing) {
        if (removedImage) {
          finalForm.image_url = null;                           // xoá ảnh
        } else {
          // không gửi image_url -> BE giữ nguyên
          const { image_url, ...rest } = finalForm;
          finalForm = rest;
        }
      } else {
        // tạo mới: nếu có sẵn URL thì gửi, không có thì bỏ qua
      ...
      // ===== Slug base =====
      if (!isEditing) {
        const { slug, ...rest } = finalForm;   // để BE tự sinh
        finalForm = rest;
      } else {
        finalForm.slug = slugify(finalForm.slug || '');
      }

      if (Object.keys(cleanTranslations).length) finalForm.translations = cleanTranslations;
        ...
        <div className="flex items-center justify-between p-4 border-b">
          ...
          <div className="flex items-center gap-3">
            <label className="inline-flex items-center gap-2 text-sm">
              <input type="checkbox" checked={autoTranslate} onChange={(e) => setAutoTranslate(e.target.checked)} disabled={isUploading} />
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-8">
          ...
          {activeTab === 'vi' && (
            <div className="space-y-6">
              {/* Parent Category */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{L('vi', 'parent')} *</label>
                <div className="relative">
                  <select
                    name="parent_id"
              ...
              {/* Name */}
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">{L('vi', 'name')} *</label>
                <input
                  name="name"
                  value={form.name}
                  onChange={handleChange}
              ...
              {/* Image (tận dụng 2 handler sẵn có) */}
              <ImagePicker
...
/** ImagePicker “dumb” component: chỉ render UI và bắn sự kiện lên parent */
function ImagePicker({ imagePreview, isUploading, onRemove, onPick }) {
  return (
    <div>
